    ...
    private async createDownloadClients(target: any, downloaders: any[]) {
        for (const downloadClient of downloaders) {
            if (downloadClient.type === 'transmission') {
                ...
                await target.apiClient.createQbittorrentClient(downloadClient.id, downloadClient.host, downloadClient.port, downloadClient.username, downloadClient.password);
            }
        }
    }
